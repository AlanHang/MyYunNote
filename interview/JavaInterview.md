...
1. 长时间锁等待以及可能会导致死锁。
2. 脏页刷盘以及readolog写入引发的磁盘I/O过载。
3. Undo日志膨胀，MVCC影响查询，事务日志过大，回滚困难。
4. 污染内存缓冲池，影响查询性能以及OOM。
5. 主从复制延迟
...
1. 基于排序键的游标分页，记录分页的值，适用于**唯一且有序**的字段
2. 利用子查询优化： 
   - 首先定位偏移量，然后用获取到的id向后查询,适用于**唯一且有序**的字段
   - 子查询只查索引值，然后进行关联
...
| :------------- | :--------------------------------------------------- |
| 配置管理       | 持久节点 + 监听器监听数据变化                        |
| 命名服务       | 存储服务名称，子节点存储ip映射                       |
| 分布式锁       | 临时顺序节点 + 监听器监听前一个节点                  |
| Leader 选举    | 临时节点 + 监听器监听节点消失                        |
| 集群成员管理   | 临时节点 + 监听器父节点的节点数变化                  |
| 分布式队列     | 顺序节点，序号最小的出列                             |
| 同步屏障       | 节点计数 + 监听器监听技术到达预值唤醒节点            |
| 服务注册与发现 | 提供者创建临时节点，消费者监听节点变化来获取可用列表 |
